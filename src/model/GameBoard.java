...
//        this.tools.add(new CommercialZoneDelimiterTool());
    ...
    public void setSelectedTool(Tool tool) {
        ...
        if (selectedTool== GameBoard.tools.get(0)) {
        	this.message="Buldozer, il permet de d�truire une construction pour 10 Ecus";
        }
        if (selectedTool== GameBoard.tools.get(1)) {
        	this.message="Pont coute 5 bois, 20 Ecus";
        }
        if (selectedTool== GameBoard.tools.get(2)) {
        	this.message="Puits : co�te 40 Ecus";
        }
        if (selectedTool== GameBoard.tools.get(3)) {
        	this.message="Maison : co�te 20 bois, 20 Ecus et permet d'acceuillir 6 habitants, ils mangent 2 rations par jour";
        }
        if (selectedTool== GameBoard.tools.get(4)) {
        	this.message="Ferme : co�te 40 bois, 20 Ecus et permet de produire 5 unit�s de nourriture par fermier";
        }
        if (selectedTool== GameBoard.tools.get(5)) {
        	this.message="Mine : co�te 40 pierre, 20 Ecus et permet de produire 5 fer par mineur";
        }
        if (selectedTool== GameBoard.tools.get(6)) {
        	this.message="Camp de b�cheron : co�te 40 �cus et permet de produire 5 bois par b�cheron";
        }
        this.notifyViews();
        //
    }
